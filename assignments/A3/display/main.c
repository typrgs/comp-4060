...
bool checkIfDataPresent()
{
  ...
  // we only want to check if the data is present if the message is from the device we want the data from (would send a query to)
  if (msg.typeID == MSG_BROADCAST && msg.deviceID == deviceIDs[currMode])
  {
    switch (currMode)
    {
      case TEMP_HUMIDITY: 
        dataPresent = msg.sensorID & HUMIDITY_DATA_MASK;
        break;

      case RAW_ANALOG: 
        dataPresent = (msg.sensorID & ANALOG_DATA_MASK);
        break;        
      
      case MAGNET_DISTANCE: 
        dataPresent = (msg.sensorID & MAGNET_DISTANCE_MASK);
        break;        
      
      case FAN_RPM: 
        dataPresent = (msg.sensorID & FAN_RPM_DATA_MASK);
        break;   
...
void updateDisplay()
{
  ...
  if (newDisplayData)
  {
    ...
    // broadcasts can have all available data so we need to get the offset for the data we currently want
    if (msg.typeID == MSG_BROADCAST)
    {
      dataPresent = checkIfDataPresent();

      switch (currMode)
      {
        case TEMP_HUMIDITY: 
          dataOffset = TEMPERATURE_OFFSET;
          break;        
        
        case RAW_ANALOG: 
          dataOffset = ANALOG_OFFSET;
          break;        
        
        case MAGNET_DISTANCE: 
          dataOffset = MAGNET_DISTANCE_OFFSET;
          break;        
        
        case FAN_RPM: 
          dataOffset = FAN_RPM_OFFSET;
         break;   
    ...
    // only update the display if the data is part of this modules broadcast
    if (dataPresent)
    {
      switch (currMode)
      {
...
void processMsg()
{
  ...
  if (msg.typeID==MSG_REPLY && msg.sensorID==currMode) 
    newDisplayData = true;
  else if (msg.typeID==MSG_BROADCAST)
  {
    newDisplayData = checkIfDataPresent();
  }
  else
    newDisplayData = false;
}
...
int main(void)
{
  ...
  spiInit();
  displayInit();
  CANInit((uint8_t *)sensorMsg, PROTOCOL_PACKET_SIZE, processMsg);
  heartInit();
