...
void requestData()
{
  ...
  uartCANSendBytes((uint8_t *)&msg, PROTOCOL_HEADER_BYTES);
  if (uartCANReceiveBytes((uint8_t *)sensorMsg, PROTOCOL_PACKET_SIZE, 100))
    processMsg();
}
...
void sendFanCommand(uint8_t newDuty)
{
  ...
  // send the request and wait for a response
  uartCANSendBytes((uint8_t *)&msg, PROTOCOL_PACKET_SIZE);
}
